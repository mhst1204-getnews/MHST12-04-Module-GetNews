...
$array_structure_image['Y_m']=$module_name.'/'.date('Y/m');$array_structure_image['Ym_d']=
$module_name.'/'.date('Y_m/d');$array_structure_image['Y_m_d']=$module_name.'/'.date('Y/m/d');
    ...
    if(!empty($e)){
      ...
      foreach($e as $p){
        if(!empty($p)and!is_dir(NV_UPLOADS_REAL_DIR.'/'.$cp.$p))
        {
            ...
            nv_loadUploadDirList(false);if($mk[0]>0){$upload_real_dir_page=$mk[2];}
        }
        elseif(!empty($p)){$upload_real_dir_page=NV_UPLOADS_REAL_DIR.'/'.$cp.$p;}
        $cp.=$p.'/';
        }
      }
...
while(list($bid_i,$adddefault_i,$title_i)=$db->sql_fetchrow($result))
    {
        ...
        if($adddefault_i){$id_block_content[]=$bid_i;}
    }
    ...
    if($rowcontent['id']>0)
    {
        ...
        if(!empty($rowcontent['id']))
        {
            ...
            {$check_permission=true;}
                ...
                $check_edit=0;$status=$rowcontent['status'];
                foreach($arr_catid as $catid_i)
                {
                    if(isset($array_cat_admin[$admin_id][$catid_i]))
                    {
                        if($array_cat_admin[$admin_id][$catid_i]['admin']==1){++$check_edit;}
                        else{if($array_cat_admin[$admin_id][$catid_i]['edit_content']==1){++$check_edit;}
                        elseif($array_cat_admin[$admin_id][$catid_i]['pub_content']==1 and($status==0 or $status=2)){++$check_edit;}
                        elseif($status==0 and $rowcontent['admin_id']==$admin_id){++$check_edit;}}
                    }
                }
                if($check_edit==sizeof($arr_catid)){$check_permission=true;}}
        }
        if(!$check_permission)
        {
...
foreach($global_array_cat as $catid_i=>$array_value)
{
    ...
    if($check_add_content){$array_cat_add_content[]=$catid_i;}
    if($check_pub_content)
    {$array_cat_pub_content[]=$catid_i;}
    if($check_edit_content){$array_cat_edit_content[]=$catid_i;
    }
}
if($nv_Request->get_int('save','post')==1)
{
    ...
    elseif($rowcontent['status']){$array_cat_check_content=$array_cat_edit_content;}
    else{$array_cat_check_content=$array_cat_add_content;}
    ...
    if($rowcontent['topicid']==0){$rowcontent['topictext']=filter_text_input('topictext','post','');
    if(!empty($rowcontent['topictext']))
    {
        list($rowcontent['topicid'])=$db->sql_fetchrow($db->sql_query("SELECT `topicid` FROM `".NV_PREFIXLANG."_news_topics` WHERE `title`=".$db->dbescape($rowcontent['topictext'])));
    }}
    ...
    else{$rowcontent['publtime']=NV_CURRENTTIME;}
    ...
    if(empty($rowcontent['title'])){$error[]=$lang_module['error_title'];}
    elseif(empty($rowcontent['listcatid'])){$error[]=$lang_module['error_cat'];}
    elseif(trim(strip_tags($rowcontent['bodyhtml']))==""){$error[]=$lang_module['error_bodytext'];}
    if(empty($error))
    {
        ...
        {
            list($weightopic)=$db->sql_fetchrow($db->sql_query("SELECT max(`weight`) FROM `".NV_PREFIXLANG."_news_topics`"));
        ...
        if(!empty($rowcontent['sourcetext']))
        {
            ...
            if(isset($url_info['scheme'])and isset($url_info['host']))
            {
                ...
                list($rowcontent['sourceid'])=$db->sql_fetchrow($db->sql_query("SELECT `sourceid` FROM `".NV_PREFIXLANG."_news_sources` WHERE `link`=".$db->dbescape($sourceid_link)));
                if(empty($rowcontent['sourceid']))
                {
                    list($weight)=$db->sql_fetchrow($db->sql_query("SELECT max(`weight`) FROM `".NV_PREFIXLANG."_news_sources`"));
                    $weight=intval($weight)+1;$query="INSERT INTO `".NV_PREFIXLANG."_news_sources` (`sourceid`, `title`, `link`, `logo`, `weight`, `add_time`, `edit_time`) VALUES (NULL, ".$db->dbescape($url_info['host']).", ".$db->dbescape($sourceid_link).", '', ".$db->dbescape($weight).", UNIX_TIMESTAMP( ), UNIX_TIMESTAMP( ))";
                    $rowcontent['sourceid']=$db->sql_query_insert_id($query);
                }
            }
            else
            {
                list($rowcontent['sourceid'])=$db->sql_fetchrow($db->sql_query("SELECT `sourceid` FROM `".NV_PREFIXLANG."_news_sources` WHERE `title`=".$db->dbescape($rowcontent['sourcetext'])));
                if(empty($rowcontent['sourceid']))
                {
                    list($weight)=$db->sql_fetchrow($db->sql_query("SELECT max(`weight`) FROM `".NV_PREFIXLANG."_news_sources`"));
                    $weight=intval($weight)+1;
                    $query="INSERT INTO `".NV_PREFIXLANG."_news_sources` (`sourceid`, `title`, `link`, `logo`, `weight`, `add_time`, `edit_time`) VALUES (NULL, ".$db->dbescape($rowcontent['sourcetext']).", '', '', ".$db->dbescape($weight).", UNIX_TIMESTAMP( ), UNIX_TIMESTAMP( ))";
                    $rowcontent['sourceid']=$db->sql_query_insert_id($query);
                }
            }
        }
        if($rowcontent['keywords']=="")
        {
            ...
            else{$rowcontent['keywords']=nv_get_keywords($rowcontent['bodyhtml']);
            }
        }
        ...
        {
            $rowcontent['homeimgfile']="";}$check_thumb=false;
            if($rowcontent['id']>0)
            {
                list($homeimgfile,$homeimgthumb)=$db->sql_fetchrow($db->sql_query("SELECT `homeimgfile`, `homeimgthumb` FROM `".NV_PREFIXLANG."_news_rows` WHERE `id`=".$rowcontent['id']));
                if($rowcontent['homeimgfile']!=$homeimgfile)
                {
                    $check_thumb=true;
                    if($homeimgthumb!=""and $homeimgthumb!="|")
                    {
                        $rowcontent['homeimgthumb']="";$homeimgthumb_arr=explode("|",$homeimgthumb);
                ...
                else{$rowcontent['homeimgthumb']=$homeimgthumb;
                }
            }
            ...
            if($check_thumb and file_exists($homeimgfile))
            {
                ...
                $basename=basename($homeimgfile);$image=new image($homeimgfile,NV_MAX_WIDTH,NV_MAX_HEIGHT);
                $thumb_basename=$basename;$i=1;
                while(file_exists(NV_ROOTDIR.'/'.NV_FILES_DIR.'/'.$module_name.'/thumb/'.$thumb_basename))
                {
                    $thumb_basename=preg_replace('/(.*)(\.[a-zA-Z]+)$/','\1_'.$i.'\2',$basename);++$i;
                }
                ...
                $block_basename=$basename;$i=1;
                while(file_exists(NV_ROOTDIR.'/'.NV_FILES_DIR.'/'.$module_name.'/block/'.$block_basename))
                {
                    $block_basename=preg_replace('/(.*)(\.[a-zA-Z]+)$/','\1_'.$i.'\2',$basename);++$i;
                }
                ...
                $image_info=$image->create_Image_info;$block_name=str_replace(NV_ROOTDIR.'/'.NV_FILES_DIR.'/'.$module_name.'/','',$image_info['src']);
                $image->close();
            ...
            if($rowcontent['id']==0)
            {
                ...
                if($rowcontent['status']==1 and $rowcontent['publtime']>NV_CURRENTTIME)
                {
                    ...
                    if($rowcontent['id']>0)
                    {
                        ...
                        $ct_query=array();
                        $tbhtml=NV_PREFIXLANG."_news_bodyhtml_".ceil($rowcontent['id']/2000);
            ...
            if($rowcontent_old['status']==1)
            {
                ...
                foreach($array_cat_new as $catid)
                {
                    ...
                    $ct_query[]=( int )$db->sql_query("UPDATE `".NV_PREFIXLANG."_news_bodytext` SET `bodytext`=".$db->dbescape_string($rowcontent['bodytext'])." WHERE `id` =".$rowcontent['id']);
                    if(array_sum($ct_query)!=sizeof($ct_query))
                    {$error[]=$lang_module['errorsave'];}}
                    else{$error[]=$lang_module['errorsave'];}$db->sql_freeresult();}
                    nv_set_status_module();if(empty($error))
                    {
                        foreach($id_block_content as $bid_i)
                        {
                            $db->sql_query("INSERT INTO `".NV_PREFIXLANG."_news_block` (`bid`, `id`, `weight`) VALUES ('".$bid_i."', '".$rowcontent['id']."', '0')");}$id_block_content[]=0;
                            $db->sql_query("DELETE FROM `".NV_PREFIXLANG."_news_block` WHERE `id` = ".$rowcontent['id']." AND `bid` NOT IN (".implode(",",$id_block_content).")");
                            $id_block_content=array_keys($array_block_cat_module);
                            foreach($id_block_content as $bid_i)
                            {nv_news_fix_block($bid_i,false);}
                            $url=NV_BASE_ADMINURL."index.php?".NV_NAME_VARIABLE."=".$module_name;$msg1=$lang_module['content_saveok'];
                            $msg2=$lang_module['content_main']." ".$module_info['custom_title'];
                            redriect($msg1,$msg2,$url);}}else{$url="javascript: history.go(-1)";
                            $msg1=implode("<br />",$error);
                            $msg2=$lang_module['content_back'];redriect($msg1,$msg2,$url);}
                            }
                        ...
                        $result=$db->sql_query($sql);$array_topic_module=array();$array_topic_module[0]=$lang_module['topic_sl'];
                        ...
                        $result=$db->sql_query($sql);$array_source_module=array();
                        $array_source_module[0]=$lang_module['sources_sl'];
                        while(list($sourceid_i,$title_i)=$db->sql_fetchrow($result)){$array_source_module[$sourceid_i]=$title_i;}
                        ...
                        {$emin=$ehour=0;$exp_date="";}
                        else{$exp_date=date("d/m/Y",$rowcontent['exptime']);
                        $tdate=date("H|i",$rowcontent['exptime']);list($ehour,$emin)=explode("|",$tdate);}
                        if($rowcontent['status']and $rowcontent['publtime']>NV_CURRENTTIME)
                        {
                            $array_cat_check_content=$array_cat_pub_content;}
                        elseif($rowcontent['status']){$array_cat_check_content=$array_cat_edit_content;}
                        ...
                        //a
                        if(empty($array_cat_check_content)){Header("Location: ".NV_BASE_ADMINURL."index.php?".NV_NAME_VARIABLE."=".$module_name."&".NV_OP_VARIABLE."=cat");die();}
                        $contents="";$my_head="<link rel=\"stylesheet\" type=\"text/css\" href=\"".NV_BASE_SITEURL."js/jquery/jquery.autocomplete.css\" />\n";$my_head.="<link type=\"text/css\" href=\"".NV_BASE_SITEURL."js/ui/jquery.ui.core.css\" rel=\"stylesheet\" />\n";$my_head.="<link type=\"text/css\" href=\"".NV_BASE_SITEURL."js/ui/jquery.ui.theme.css\" rel=\"stylesheet\" />\n";$my_head.="<link type=\"text/css\" href=\"".NV_BASE_SITEURL."js/ui/jquery.ui.datepicker.css\" rel=\"stylesheet\" />\n";$my_head.="<script type=\"text/javascript\" src=\"".NV_BASE_SITEURL."js/jquery/jquery.autocomplete.js\"></script>\n";$my_head.="<script type=\"text/javascript\" src=\"".NV_BASE_SITEURL."js/ui/jquery.ui.core.min.js\"></script>\n";$my_head.="<script type=\"text/javascript\" src=\"".NV_BASE_SITEURL."js/ui/jquery.ui.datepicker.min.js\"></script>\n";$my_head.="<script type=\"text/javascript\" src=\"".NV_BASE_SITEURL."js/language/jquery.ui.datepicker-".NV_LANG_INTERFACE.".js\"></script>\n";$xtpl=new XTemplate("content.tpl",NV_ROOTDIR."/themes/".$global_config['module_theme']."/modules/".$module_file);$xtpl->assign('LANG',$lang_module);$xtpl->assign('rowcontent',$rowcontent);$xtpl->assign('NV_BASE_ADMINURL',NV_BASE_ADMINURL);$xtpl->assign('NV_NAME_VARIABLE',NV_NAME_VARIABLE);$xtpl->assign('NV_OP_VARIABLE',NV_OP_VARIABLE);$xtpl->assign('module_name',$module_name);foreach($global_array_cat as $catid_i=>$array_value){if(defined('NV_IS_ADMIN_MODULE')){$check_show=1;}else{$array_cat=GetCatidInParent($catid_i);$check_show=array_intersect($array_cat,$array_cat_check_content);}if(!empty($check_show)){$space=intval($array_value['lev'])*30;$catiddisplay=(sizeof($array_catid_in_row)>1 and(in_array($catid_i,$array_catid_in_row)))?'':' display: none;';$temp=array('catid'=>$catid_i,"space"=>$space,"title"=>$array_value['title'],"disabled"=>(!in_array($catid_i,$array_cat_check_content))?" disabled=\"disabled\"":"","checked"=>(in_array($catid_i,$array_catid_in_row))?" checked=\"checked\"":"","catidchecked"=>($catid_i==$rowcontent['catid'])?" checked=\"checked\"":"","catiddisplay"=>$catiddisplay);$xtpl->assign('CATS',$temp);$xtpl->parse('main.catid');}}$checkcop=($rowcontent['copyright'])?" checked=\"checked\"":"";$xtpl->assign('checkcop',$checkcop);while(list($topicid_i,$title_i)=each($array_topic_module)){$sl=($topicid_i==$rowcontent['topicid'])?" selected=\"selected\"":"";$xtpl->assign('topicid',$topicid_i);$xtpl->assign('topic_title',$title_i);$xtpl->assign('sl',$sl);$xtpl->parse('main.rowstopic');}while(list($id_imgposition,$title_imgposition)=each($array_imgposition)){$sl=($id_imgposition==$rowcontent['imgposition'])?" selected=\"selected\"":"";$xtpl->assign('id_imgposition',$id_imgposition);$xtpl->assign('title_imgposition',$title_imgposition);$xtpl->assign('posl',$sl);$xtpl->parse('main.looppos');}$xtpl->assign('publ_date',$publ_date);$select="";for($i=0;$i<=23;++$i){$select.="<option value=\"".$i."\"".(($i==$phour)?" selected=\"selected\"":"").">".str_pad($i,2,"0",STR_PAD_LEFT)."</option>\n";}$xtpl->assign('phour',$select);$select="";for($i=0;$i<60;++$i){$select.="<option value=\"".$i."\"".(($i==$pmin)?" selected=\"selected\"":"").">".str_pad($i,2,"0",STR_PAD_LEFT)."</option>\n";}$xtpl->assign('pmin',$select);$xtpl->assign('exp_date',$exp_date);$select="";for($i=0;$i<=23;++$i){$select.="<option value=\"".$i."\"".(($i==$ehour)?" selected=\"selected\"":"").">".str_pad($i,2,"0",STR_PAD_LEFT)."</option>\n";}$xtpl->assign('ehour',$select);$select="";for($i=0;$i<60;++$i){$select.="<option value=\"".$i."\"".(($i==$emin)?" selected=\"selected\"":"").">".str_pad($i,2,"0",STR_PAD_LEFT)."</option>\n";}$xtpl->assign('emin',$select);$select="";while(list($commid_i,$commid_title_i)=each($array_allowed_comm)){$comm_sl=($commid_i==$rowcontent['allowed_comm'])?" selected=\"selected\"":"";$select.="<option value=\"".$commid_i."\" ".$comm_sl.">".$commid_title_i."</option>\n";}$xtpl->assign('allowed_comm',$select);$select="";while(list($sourceid_i,$source_title_i)=each($array_source_module)){$source_sl=($sourceid_i==$rowcontent['sourceid'])?" selected=\"selected\"":"";$select.="<option value=\"".$sourceid_i."\" ".$source_sl.">".$source_title_i."</option>\n";}$xtpl->assign('sourceid',$select);
...
{$ch=in_array($bid_i,$id_block_content)?" checked=\"checked\"":"";$shtm.="<tr><td><input class=\"news_checkbox\" type=\"checkbox\" name=\"bids[]\" value=\"".$bid_i."\"".$ch." />".$bid_title."</td></tr>\n";}
...
if($error!="")
{$xtpl->assign('error',implode("<br />",$error));$xtpl->parse('main.error');}
if($rowcontent['status']==1 and $rowcontent['id']>0)
{$xtpl->parse('main.status');}
else{$xtpl->parse('main.status0');}
